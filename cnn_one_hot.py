...
def create_vector_one_hot_from_class(classification_value):
    vec = np.zeros(CLASSIFICATION_MAX, dtype=np.float32)
    vec[int(classification_value)-1] = 1
    return np.asarray(vec, dtype=np.float32)


def create_vector_one_hot_from_char(char):
    vec = np.zeros(ALPHABET_SIZE, dtype=np.float32)
...
def load_data(filename, corpus_length):
    ...
    with open(filename, 'rb') as csvfile:
        ...
        while tmp_compteur < corpus_length:
            ...
            vec_corpus_y.append(create_vector_one_hot_from_class(row[INDEX_CLASS_IN_CORPUS]))
            row[INDEX_SENTENCE_IN_CORPUS] = str(row[INDEX_SENTENCE_IN_CORPUS])
            for letter in row[INDEX_SENTENCE_IN_CORPUS][:SENTENCE_LENGTH]:
                vec_sentence.append(create_vector_one_hot_from_char(letter))
...
def create_cnn_model():
    ...
    conv_1 = Convolution1D(NUMBER_OF_CONVOLUTION_KERNELS, 7, input_shape=(SENTENCE_LENGTH, ALPHABET_SIZE))
    model.add(conv_1)
